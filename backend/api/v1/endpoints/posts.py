...
async def process_post(
    ...
    try:
        ...
        logger.info(f"Processing post {request.post_id}: {len(request.content)} chars, {len(request.image_urls or [])} images, {len(request.video_urls or [])} videos")
        ...
        raise HTTPException(
            ...
            detail=f"Failed to process post: {str(e)}"
        )
