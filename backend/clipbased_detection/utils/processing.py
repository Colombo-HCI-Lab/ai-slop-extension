...
def make_pre(opt):
    ...
    if opt.loadSize > 0:
        ...
        transforms_list.append(CenterCropPad(opt.loadSize, pad_if_needed=True, padding_mode="symmetric"))
...
def make_post(opt):
    ...
    if opt.resizeSize > 0:
        print("\nUsing Post Resizing\n")
        transforms_list.append(transforms.Resize(opt.resizeSize, interpolation=transforms.InterpolationMode.BICUBIC))
    ...
    if opt.cropSize > 0:
        if not opt.no_random_crop:
            ...
            transforms_list.append(transforms.RandomCrop(opt.cropSize, pad_if_needed=True, padding_mode="symmetric"))
        else:
            print("\nUsing Post Central Crop\n")
            transforms_list.append(CenterCropPad(opt.cropSize, pad_if_needed=True, padding_mode="symmetric"))
...
def make_aug(opt):
    ...
    if opt.jitter_prob > 0:
        transforms_list_aug.append(transforms.RandomApply([transforms.ColorJitter(0.4, 0.4, 0.4, 0.1)], p=opt.jitter_prob))
    ...
    if opt.blur_prob > 0:
        transforms_list_aug.append(transforms.Lambda(lambda img: data_augment_blur(img, opt.blur_prob, opt.blur_sig)))

    if opt.cmp_prob > 0:
        transforms_list_aug.append(transforms.Lambda(lambda img: data_augment_cmp(img, opt.cmp_prob, opt.cmp_method, opt.cmp_qual)))

    if opt.rot90_prob > 0:
        transforms_list_aug.append(transforms.Lambda(lambda img: data_augment_rot90(img, opt.rot90_prob)))

    if opt.hpf_prob > 0:
        transforms_list_aug.append(transforms.ToTensor())
        transforms_list_aug.append(transforms.Lambda(lambda img: data_augment_hpf(img, opt.hpf_prob, opt.blur_sig)))
...
def make_normalize(norm_type):
    ...
    if norm_type == "resnet":
        ...
        transforms_list.append(lambda x: x[..., : (x.shape[-2] // 2 * 2), : (x.shape[-1] // 2 * 2)])
        transforms_list.append(
            lambda x: (x - interpolate(x[None, ..., ::2, ::2], scale_factor=2.0, mode="nearest", recompute_scale_factor=True)[0])
            * 2.0
            / 3.0
        )
    elif norm_type == "cooc":
...
def create_noise_transforms(p, var_limit=(10.0, 50.0)):
    ...
    return transforms.Lambda(lambda img: Image.fromarray(aug(image=np.array(img))["image"]))
...
def normalization_cooc(pic):
    ...
    bins = np.arange(257)
    H = np.stack(
        [np.histogram2d(x[:, :, i].flatten(), y[:, :, i].flatten(), bins, density=True)[0] for i in range(x.shape[2])],
        0,
    )
...
class CenterCropPad:
    def __init__(self, siz, pad_if_needed=False, padding_fill=0, padding_mode="constant"):
