...
class MediaProcessingRegistry:
    ...
    def register_media(self, post_id: str, media_url: str, media_type: str) -> str:
        ...
        if media_key not in self._registry:
            self._registry[media_key] = MediaProcessingRecord(
                post_id=post_id,
                media_url=media_url,
                media_type=media_type
            )
            logger.debug("Registered new media for processing", 
                        media_key=media_key, media_type=media_type)
    ...
    def update_processing_stage(self, media_key: str, stage: str, **kwargs) -> None:
        ...
        if media_key in self._registry:
            ...
            logger.debug("Updated media processing stage",
                        media_key=media_key, old_stage=old_stage, new_stage=stage)
    ...
    def is_already_processed(self, post_id: str, media_url: str, min_stage: str = "downloaded") -> bool:
        ...
        try:
            ...
            logger.debug("Checked media processing status",
                        media_key=media_key, current_stage=record.processing_stage,
                        min_stage=min_stage, is_processed=is_processed)
            
            return is_processed
        except ValueError as e:
            logger.error("Invalid stage in processing check", 
                        media_key=media_key, stage=record.processing_stage, error=str(e))
            return False
