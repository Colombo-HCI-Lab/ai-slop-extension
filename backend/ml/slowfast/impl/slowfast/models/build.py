...
def build_model(cfg, gpu_id=None):
    ...
    if cfg.BN.NORM_TYPE == "sync_batchnorm_apex":
        ...
        process_group = apex.parallel.create_syncbn_process_group(group_size=cfg.BN.NUM_SYNC_DEVICES)
    ...
    if cfg.NUM_GPUS > 1:
        ...
        model = torch.nn.parallel.DistributedDataParallel(
            ...
            find_unused_parameters=(True if cfg.MODEL.DETACH_FINAL_FC or cfg.MODEL.MODEL_NAME == "ContrastiveModel" else False),
        )
        if cfg.MODEL.FP16_ALLREDUCE:
            model.register_comm_hook(state=None, hook=comm_hooks_default.fp16_compress_hook)
    return model
