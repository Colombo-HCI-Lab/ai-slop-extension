...
class MaskMViT(MViT):
    ...
    def _mae_random_masking(self, x, mask_ratio, mask_in=None):
        ...
        if mask_in is None:
            if self.cfg.AUG.MASK_TUBE:
                noise = torch.rand(N, 1, self.H * self.W, device=x.device).repeat([1, self.T, 1]).reshape(N, L)  # noise in [0, 1]
            else:
    ...
    def _mae_forward_encoder(self, x, mask_ratio, mask=None):
        ...
        x, mask, ids_restore, ids_keep = self._mae_random_masking(x, mask_ratio, None if self.cfg.MASK.MAE_RND_MASK else mask)
        if self.cfg.MASK.PER_FRAME_MASKING:
            x = x.view([B, -1, C])
    ...
    def _mae_visualize(self, imgs, pred, mask):
        ...
        reconstruct = self._unpatchify(
            ...
            + self._patchify(im_viz, time_stride_loss=self.cfg.MASK.TIME_STRIDE_LOSS) * (1 - mask.reshape(N, t * h * w, 1))
        )
        ...
        for i in range(comparison.shape[0]):
            misc.plot_input_normed(
                ...
                + "/vis_mask/vid/{}vis_video_in_mask_out_mr{}/vis_{}_{}.mp4".format(pfx[pfx.rfind("/") + 1 : -5], mr, self.counter, i),
                folder_path=self.cfg.OUTPUT_DIR + "/vis_mask/vid/{}vis_video_in_mask_out_mr{}".format(pfx[pfx.rfind("/") + 1 : -5], mr),
                make_grids=True,
                output_video=True,
            )
        return pred[0]
