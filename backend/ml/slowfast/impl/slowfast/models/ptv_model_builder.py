...
def get_head_act(act_func):
    ...
    if act_func == "softmax":
        ...
        raise NotImplementedError("{} is not supported as a head activation function.".format(act_func))
...
class PTVResNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        assert cfg.RESNET.TRANS_FUNC == "bottleneck_transform", f"Unsupported TRANS_FUNC type {cfg.RESNET.TRANS_FUNC} for PTVResNet"
...
class PTVSlowFast(nn.Module):
    def __init__(self, cfg):
        ...
        assert cfg.RESNET.TRANS_FUNC == "bottleneck_transform", f"Unsupported TRANS_FUNC type {cfg.RESNET.TRANS_FUNC} for PTVSlowFast"
        ...
        for pathway in range(2):
            for stage in range(4):
                stage_conv_a_kernel_sizes[pathway].append(
                    ((temp_kernel[stage + 1][pathway][0], 1, 1),) * num_block_temp_kernel[stage][pathway]
                    + ((1, 1, 1),) * (stage_depth[stage] - num_block_temp_kernel[stage][pathway])
                )
...
class PTVX3D(nn.Module):
    def __init__(self, cfg):
        ...
        assert cfg.RESNET.TRANS_FUNC == "x3d_transform", f"Unsupported TRANS_FUNC type {cfg.RESNET.TRANS_FUNC} for PTVX3D"
        assert cfg.DETECTION.ENABLE is False, "Detection model is not supported for PTVX3D yet."
...
class PTVCSN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        assert cfg.DETECTION.ENABLE is False, "Detection model is not supported for PTVCSN yet."
...
class PTVR2plus1D(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        assert cfg.DETECTION.ENABLE is False, "Detection model is not supported for PTVR2plus1D yet."
...
class PTVMViT(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        assert cfg.DETECTION.ENABLE is False, "Detection model is not supported for PTVMViT yet."
