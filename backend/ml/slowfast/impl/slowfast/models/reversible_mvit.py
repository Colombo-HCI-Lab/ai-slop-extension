...
class ReversibleMViT(nn.Module):
    ...
    def __init__(self, config, model):
        ...
        for i in range(depth):
            ...
            embed_dim = round_width(embed_dim, dim_mul[i - 1] if i > 0 else 1.0, divisor=num_heads)
            dim_out = round_width(
                embed_dim,
                dim_mul[i],
            ...
            dimout_correction = 2 if (input_mult == 2 and "concat" in self.pre_q_fusion) else 1

            self.layers.append(
                layer_type(
                    dim=embed_dim * input_mult,  # added only for concat fusion before Qpooling layers
...
class ReversibleBlock(nn.Module):
    ...
    def backward_pass(
        ...
        with torch.enable_grad():
            ...
            torch.manual_seed(self.seeds["droppath"])
            g_Y_1 = drop_path(g_Y_1, drop_prob=self.drop_path_rate, training=self.training)
        ...
        with torch.enable_grad():
            ...
            torch.manual_seed(self.seeds["droppath"])
            f_X_2 = drop_path(f_X_2, drop_prob=self.drop_path_rate, training=self.training)
