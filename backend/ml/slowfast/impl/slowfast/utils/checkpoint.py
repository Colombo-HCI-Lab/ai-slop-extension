...
def inflate_weight(state_dict_2d, state_dict_3d):
    ...
    for k, v2d in state_dict_2d.items():
        ...
        if len(v2d.shape) == 4 and len(v3d.shape) == 5:
            ...
            logger.info("Unexpected {}: {} -|> {}: {}".format(k, v2d.shape, k, v3d.shape))
            ...
            if converted_key in ms.state_dict():
                ...
                if (
                    ...
                    caffe2_checkpoint["blobs"][key] = np.concatenate(
                        [caffe2_checkpoint["blobs"][key]] * (model_blob_shape[0] // c2_blob_shape[0])
                    )
                    c2_blob_shape = caffe2_checkpoint["blobs"][key].shape
        ...
        if inflation:
            ...
            if image_init:
                ...
                if "patch_embed.proj.weight" in pre_train_dict.keys() and "patch_embed.proj.weight" in model_dict.keys():
                    ...
                    if (
                        len(pre_train_dict["patch_embed.proj.weight"].shape) == 4 and len(model_dict["patch_embed.proj.weight"].shape) == 5
                    ):  # img->video
                        t = model_dict["patch_embed.proj.weight"].shape[2]
                        pre_train_dict["patch_embed.proj.weight"] = pre_train_dict["patch_embed.proj.weight"][:, :, None, :, :].repeat(
                            1, 1, t, 1, 1
                        )
                        logger.info(f"inflate patch_embed.proj.weight to {pre_train_dict['patch_embed.proj.weight'].shape}")
                    elif (
                        len(pre_train_dict["patch_embed.proj.weight"].shape) == 5 and len(model_dict["patch_embed.proj.weight"].shape) == 4
                    ):  # video->img
                        ...
                        pre_train_dict["patch_embed.proj.weight"] = pre_train_dict["patch_embed.proj.weight"].sum(2)  # take avg
                        logger.info(
                        ...
                        if "pos_embed_spatial" in pre_train_dict.keys() and "pos_embed" in model_dict.keys():
                            ...
                            if "pos_embed_class" in pre_train_dict.keys() and pos_embds.shape != model_dict["pos_embed"].shape:
                                pos_embds = torch.cat(
                                    [
                ...
                if k in model_dict:
                    if v.size() == model_dict[k].size():
                        ...
                        if "attn.rel_pos" in k:
                            ...
                            v = v[0].t()
                            pre_train_dict_match[k] = v
                            logger.info("{} reshaped from {} to {}".format(k, v_shape, v.shape))
                            ...
                            logger.info("{} reshaped from {} to {}".format(k, v_shape, pre_train_dict_match[k].shape))
