...
class GCSStorageService:
    ...
    def __init__(self):
        ...
        if not settings.gcs_bucket_name:
            raise ValueError(
                "GCS_BUCKET_NAME is required for media storage. "
        ...
        try:
            ...
            logger.info("GCS storage service initialized successfully", 
    ...
    async def upload_media(self, data: bytes, gcs_path: str, content_type: str) -> str:
        ...
        try:
            ...
            logger.info("Successfully uploaded media to GCS", 
                       gcs_path=gcs_path, 
                       size_bytes=len(data),
    ...
    async def media_exists(self, gcs_path: str) -> bool:
        ...
        try:
            ...
            logger.warning("Error checking GCS file existence", 
                          gcs_path=gcs_path, 
                          error=str(e))
            return False
    ...
    async def download_media(self, gcs_path: str) -> bytes:
        ...
        try:
            ...
            logger.debug("Downloaded media from GCS", 
                        gcs_path=gcs_path, 
                        size_bytes=len(data))
            return data
            ...
            logger.error("Unexpected error during GCS download", 
                        gcs_path=gcs_path, 
                        error=str(e), 
                        exc_info=True)
            raise
    ...
    async def delete_media(self, gcs_path: str) -> bool:
        ...
        try:
            ...
            logger.error("GCS deletion failed", 
                        gcs_path=gcs_path, 
                        error=str(e), 
                        exc_info=True)
            return False
        except Exception as e:
            logger.error("Unexpected error during GCS deletion", 
                        gcs_path=gcs_path, 
                        error=str(e), 
                        exc_info=True)
            return False
            ...
            for blob in blobs:
                if blob.time_created < cutoff_time:
                    try:
                        blob.delete()
                        deleted_count += 1
                        logger.debug("Deleted old GCS file", 
                                   name=blob.name, 
                                   created=blob.time_created)
                    except Exception as e:
                        logger.warning("Failed to delete old GCS file", 
                                     name=blob.name, 
                                     error=str(e))
    ...
    def get_public_url(self, gcs_path: str) -> Optional[str]:
        ...
        try:
            ...
            logger.warning("Error getting public URL", 
                          gcs_path=gcs_path, 
                          error=str(e))
            return None