...
class GeminiOnDemandService:
    ...
    async def ensure_gemini_uri_exists(
        self, 
        post_id: str, 
        media_url: str, 
        db: AsyncSession
        ...
        try:
            ...
            if storage_path:
                ...
                if gemini_uri:
                    ...
                    logger.info("On-demand Gemini upload successful", 
                              post_id=post_id, gemini_uri=gemini_uri)
                
                return gemini_uri
            
            logger.warning("No storage path available for on-demand upload", 
                         post_id=post_id, media_url=media_url)
            return None
        
        except Exception as e:
            logger.error("Error in on-demand Gemini upload", 
                       post_id=post_id, media_url=media_url, error=str(e))
            return None
    
    async def batch_ensure_gemini_uris(
        self, 
        post_id: str, 
        media_urls: List[str], 
        db: AsyncSession
        ...
        logger.info(
            "Batch on-demand Gemini upload completed",
            post_id=post_id,
            requested_count=len(media_urls),
            successful_count=len(uris)
        )
