...
class GeminiRecoveryService:
    ...
    async def recover_missing_gemini_uris(
        self, 
        post_id: str, 
        db: AsyncSession
    ) -> Dict[str, str]:
        ...
        try:
            ...
            logger.info(
                "Found media with missing Gemini URIs, attempting recovery",
                post_id=post_id,
                missing_count=len(media_missing_gemini)
            )
                ...
                try:
                    ...
                    if media.storage_path.startswith("gs://"):
                        ...
                        if await gcs_service.media_exists(gcs_path):
                            ...
                            gemini_uri = await self._upload_to_gemini_from_storage(
                                media.storage_path,
                                mime_type,
                                display_name,
                                gcs_service
                            ...
                            logger.warning(
                                "Storage file missing, cannot recover Gemini URI",
                                post_id=post_id,
                                storage_path=media.storage_path
                            )
                    ...
                    logger.error(
                        "Failed to recover Gemini URI for media",
                        post_id=post_id,
                        media_id=media.id,
                        error=str(e)
                    )
    ...
    async def _upload_to_gemini_from_storage(
        self,
        storage_path: str,
        mime_type: str,
        display_name: str,
        ...
        try:
            ...
            logger.error("Failed to upload to Gemini from storage", 
