...
class PostMediaService:
    ...
    async def _detect_media_changes(
        self, post_id: str, new_image_urls: List[str], new_video_urls: List[str], db: AsyncSession
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            existing_result = await db.execute(select(PostMedia).where(PostMedia.post_id == post_id))
            existing_media = existing_result.scalars().all()

            # Separate existing URLs by type
            existing_image_urls = {media.media_url for media in existing_media if media.media_type == "image"}
            existing_video_urls = {media.media_url for media in existing_media if media.media_type == "video"}
            ...
            unchanged_media = [media for media in existing_media if media.media_url in unchanged_urls]
            ...
            return {"unchanged_media": [], "urls_to_remove": [], "urls_to_add": new_image_urls + new_video_urls, "needs_update": True}
    ...
    async def _save_media_urls(
        ...
        changes = await self._detect_media_changes(post_id, request.image_urls or [], request.video_urls or [], db)
        ...
        if changes["urls_to_remove"]:
            await db.execute(
                PostMedia.__table__.delete().where(PostMedia.post_id == post_id).where(PostMedia.media_url.in_(changes["urls_to_remove"]))
            )
            logger.info("Removed obsolete media records", post_id=post_id, removed_count=len(changes["urls_to_remove"]))
        ...
        if changes["urls_to_add"]:
            ...
            await self._create_media_entries_for_new_urls(post_id, new_image_urls, new_video_urls, gemini_file_uris, media_file_info, db)
