...
class ResNetBasicHead(nn.Module):
    ...
    def __init__(
        ...
        if cfg.CONTRASTIVE.NUM_MLP_LAYERS == 1:
            ...
            self.projection = MLPHead(
                ...
                bn_sync_num=(cfg.BN.NUM_SYNC_DEVICES if cfg.CONTRASTIVE.BN_SYNC_MLP else 1),
    ...
    def forward(self, inputs):
        ...
        if x.shape[1:4] == torch.Size([1, 1, 1]) and self.cfg.MODEL.MODEL_NAME == "ContrastiveModel":
            x = x.view(x.shape[0], -1)
...
class MSSeparateHead(nn.Module):
    ...
    def __init__(
        ...
        for depth, num_class, feature_size in zip(depth_list, num_classes, feat_sz):
            head_dim = cfg.MASK.DECODER_EMBED_DIM if cfg.MASK.MAE_ON else blocks[depth].dim_out
            op = []
